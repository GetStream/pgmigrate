...
func NewRootCommand() *cobra.Command {
	...
	flags.Int64Var(&cfg.ReplayBatchBytes, "replay-batch-bytes", cfg.ReplayBatchBytes, "decoded CDC payload per replay scheduling slice; four slices form the default durable wave")
	flags.IntVar(&cfg.ReplayBatchChanges, "replay-batch-changes", cfg.ReplayBatchChanges, "row changes per replay scheduling slice; four slices form the default durable wave")
	...
	flags.DurationVar(&cfg.VerifyConvergeTimeout, "verify-converge-timeout", cfg.VerifyConvergeTimeout, "how long a differing heap-sample row is given to settle before it is reported")
